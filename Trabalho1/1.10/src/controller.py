...
from bSplineSurface3D import BSplineSurface3D
...
class Controller():
    ...
    def drawBezierSurfaceEvent(self):
        self.form_window_bezier_surface = QWidget()
        self.form_window_bezier_surface.setMinimumSize(400, 300)

        layout = QVBoxLayout()
        label = QLabel("Enter Bézier surface patches (4x4 points per patch, lines separated by ';', patches separated by newline):")
        layout.addWidget(label)

        example = QLabel("Ex:\n(-150,-150,500),(-50,-200,580),(50,-200,580),(150,-150,500);(-180,-50,550),(-60,-60,650),(60,-60,650),(180,-50,550);(-180,50,550),(-60,60,650),(60,60,650),(180,50,550);(-150,150,500),(-50,200,580),(50,200,580),(150,150,500)")
        example.setTextInteractionFlags(Qt.TextSelectableByMouse)
        layout.addWidget(example)
        ...
        def create_object():
            self.form_window_bezier_surface.close()
            text = input_field.text()
            try:
                patches = BezierSurface3D.from_text_input(text)
                if not patches:
                    raise ValueError("No valid patches")
                obj = BezierSurface3D(patches)
                obj.color = QColorDialog().getColor()
                self.__viewport.objects.append(obj)
                obj.draw(self.__viewport)
                self.addTree(obj)
            except:
        ...
        button.clicked.connect(create_object)
        layout.addWidget(button)
        self.form_window_bezier_surface.setLayout(layout)
        self.form_window_bezier_surface.show()

    def drawBSplieneSurfaceEvent(self):
        self.form_window_bspline_surface = QWidget()
        self.form_window_bspline_surface.setMinimumSize(400, 300)

        layout = QVBoxLayout()
        label = QLabel("Enter B-spline surface control points (from 4x4 up to 20x20, lines separated by ';', rows separated by newlines):"
)
        layout.addWidget(label)
        ...
        self.form_window_bspline_surface.setLayout(layout)
        self.form_window_bspline_surface.show()

        def create_object():
            self.form_window_bspline_surface.close()
            text = input_field.text()
            # try:
            patches = BSplineSurface3D.from_text_input(text)
            # if not patches:
            #     raise ValueError("No valid patches")
    ...
    def zoomInEvent(self, window):
        const.CAM_DISTANCE *= self.zoomIn_mutiplier

        for obj in self.__viewport.objects:
            if(isinstance(obj, Object3D)):
                window.normalize(obj, [[0, 0, 0], [1, 1, 1], [0, 0, 0]])
            else:
                window.normalize(obj, self.normalizedInfos[IN])

    def zoomOutEvent(self, window):
        const.CAM_DISTANCE *= self.zoomOut_multiplier

        for obj in self.__viewport.objects:
            if(isinstance(obj, Object3D)):
                window.normalize(obj, [[0, 0, 0], [1, 1, 1], [0, 0, 0]])
            else:
                window.normalize(obj, self.normalizedInfos[OUT])
    ...
    def translateEvent(self, window):
        if self.__tree.selectedIndexes():
            ...
            if previous_on_screen:
                if isinstance(graphicObject, (Polygon, Curve, Object3D, BezierSurface3D, BSplineSurface3D)):
    ...
    def executeRotWorld(self, object, index, window):
        ...
        try:
            ...
            if (previous_on_screen):
                if isinstance(object, (Polygon, Curve, Object3D, BezierSurface3D, BSplineSurface3D)):
