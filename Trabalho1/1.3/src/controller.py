    ...
    def saveToObj(self, filename, index=None):
        with open(filename, 'w') as file:
            ...
            if index is not None:
                ...
                descritor = DescriptorOBJ("obj_selected", obj_type)
                for point in obj.points:
                    descritor.add_vertex(point[0], point[1])
                ...
                for idx, obj in enumerate(self.__viewport.objects):
                    ...
                    descritor = DescriptorOBJ(f"obj_{idx}", obj_type)
                    for point in obj.points:
                        descritor.add_vertex(point[0], point[1])
            ...
            with open(filename, 'r') as file:
                ...
                for line in file:
                    ...
                    if (parts[0] == 'o' or not line) and current_vertices:
                        self._create_object_from_data(current_vertices, current_edges)
                        current_vertices = []
                        current_edges = []
                if current_vertices:
                    self._create_object_from_data(current_vertices, current_edges)
